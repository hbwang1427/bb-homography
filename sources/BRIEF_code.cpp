...
void BRIEF_Graph(const string &model_image,const string &video_file,const bool Graph_Flag,const string &featureName)
{
	...
	int num = 32;// length of descriptor
	//FastFeatureDetector detector(500); 
	SurfFeatureDetector detector(500);
	BriefDescriptorExtractor extractor(num); //this is really num x 8 matches since they are binary matches packed into bytes
	...
	while(true)
	{
		...
		if (Graph_Flag==true)
		{			
			flag_homography = Original_BRIEF(descriptorsA,descriptorsB,keypointsA,keypointsB,final_matches,homo,final_inliers_numbers);				
			if (!flag_homography)
			{					
				final_matches.clear();
		...
		line(frame,dst_cornor[0],dst_cornor[1],Scalar(0,0,255),5);
