...
void FREAK_Graph(const string &model_image,const string &video_file,const bool Graph_Flag,const string &featureName)
{
	...
	BRISK brisk;  // use brisk to extract keypoints
	brisk(image1, Mat(), keypointsA); // only keypoints	
	...
	{								
		...
		brisk(image2, Mat(), keypointsB);    // use brisk to extract keypoints
		...
		if (Graph_Flag==true)
		{			
			flag_homography = Original_FREAK(descriptorsA,descriptorsB,keypointsA,keypointsB,final_matches,homo,final_inliers_numbers);				
			if (!flag_homography)
			{					
				final_matches.clear();
...
bool Original_FREAK(Mat&descriptorsA, Mat &descriptorsB,vector<KeyPoint>&keypointsA, 
